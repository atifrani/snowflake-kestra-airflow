...
CREATE OR REPLACE PROCEDURE CREATE_RAW_LISTINGS_PROC ( )
    RETURNS STRING NOT NULL
    LANGUAGE JAVASCRIPT
    AS
        $$
        -- create table
        var create_stmt = " CREATE OR REPLACE TABLE airbnb.bronze.raw_listings (id integer, listing_url string, name string, room_type string, minimum_nights integer, host_id integer, price string, created_at datetime, updated_at datetime);"
        -- create stram
        var stream_stmt = " CREATE OR REPLACE STREAM airbnb.bronze.raw_listings_stream on table tasks.bronze.raw_hosts;"
        -- load csv file
...
CREATE OR REPLACE TASK CREATE_RAW_LISTINGS_TASK
WAREHOUSE = COMPUTE_WH
SCHEDULE = '10 MINUTE'
AS CALL  CREATE_RAW_LISTINGS_PROC ();
